...
object DataProducer extends LogSupport {
  ...
  def dataProducer(num: Int): Array[String] = {
    val random = new Random
    val ueId = Array("10001", "10002", "10003", "10004", "10005", "10006",
      "10007", "10008", "10009", "10010", "10011", "10012", "10013", "10014")
    val text = Array("aaaaaaaaaaa", "bbbbbbbbb", "cccccccccccc", "dddddddddddd",
      "fffffffffffffff", "gggggggggggg", "hhhhhhhhhhhhh")
    ...
    (0 until num).foreach {
      x =>
        ...
        val record = s"${TimePara.getCurrentTime},${ueId(factor % ueId.length)},${text(factor % text.length)}," +
          s"${region_id_c + factor},${x_offset_c + factor + x},${y_offset_c + factor + x}\n"
